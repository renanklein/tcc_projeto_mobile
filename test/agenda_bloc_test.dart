...
void main(){
  ...
  group("AgendaLoad event", (){
    test("When event is AgendaLoad and no exception was thrown, it should emit AgendaLoadSuccess", (){
      var expectedStates = [
        AgendaInitial(),
        AgendaLoading(),
        AgendaLoadSuccess(Map())
      ];

      when(fakeAgendaRepository.getEvents()).thenAnswer((_) => Future.value(Map()));
    ...
    test("When event is AgendaLoad and an exception was thrown, it shoulb be AgendaLoadFail", (){
      var expectedStates = [
        AgendaInitial(),
        AgendaLoading(),
        AgendaLoadFail()
  ...
  group("AgendaEventAvailableTimeLoad event", (){
    test("When event is AgendaEventAvailableTimeLoad and no exception was thrown, it should emit AgendaAvailableTimeSuccess",(){
      var expectedStates = [
        AgendaInitial(),
      ...
      when(fakeAgendaRepository.getOccupedDayTimes("day")).thenAnswer((_) => Future.value(["day", "day2"]));

      expectLater(agendaBloc, emitsInOrder(expectedStates));

      agendaBloc.add(AgendaEventAvailableTimeLoad(
        day: DateTime(2020, 05, 20)
      ));
    });
  });
