...
void main() {
  ...
  group("LoginButtonPressed event", () {
    test(
        "When event is LoginButtonPressed and no operation had failed, it should emit LoginSucceded",
        () {
      var expectedStates = [LoginProcessing(), LoginSucceded()];

      expectLater(loginBloc, emitsInOrder(expectedStates));

      loginBloc.add(
          LoginButtonPressed(context: fakeContext, email: "", password: ""));
    });

    test(
        "When event is LoginButtonPressed and any of external call fails, so i should emit LoginFailure",
        () {
      var expectedStates = [LoginProcessing(), LoginFailure()];
  ...
  group("LoginResetPasswordButtonPressed event", () {
    test(
        "When event is Reset Password and no external operation had failed, so it should emit LoginPasswordReset",
        () {
      var expectedStates = [LoginPasswordReset()];
